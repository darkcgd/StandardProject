...
/**
 * Created by chenguangda on 2016/7/15.
 */
import java.io.File;
...
//        saveCrashInfo2Service(ex);
    ...
    public void collectDeviceInfo(Context ctx) {
        ...
        for (Field field : fields) {
            try {
                ...
                Log.d(TAG, field.getName().toLowerCase() + " : " + field.get(null));
            } catch (Exception e) {
    ...
    private void saveCrashInfo2Service(Throwable ex) {
        ...
        if(result!=null){
            infos.put("exceptioninfo",result);
        }else{
            infos.put("exceptioninfo",result);
        }
        Gson gson = new Gson();
        String jsonStr = gson.toJson(infos);
        send2Service(jsonStr);

    }

    private void send2Service(String errorMsg) {
        String url = Config.getCrashApi("exception/saveException");
        AbRequestParams params = new AbRequestParams();
        params.put("content", errorMsg);//异常信息
